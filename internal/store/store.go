...
func (s *Store) InsertIncoming(ctx context.Context, m Message) (int64, error) {
	...
	err := s.pool.QueryRow(ctx,
		`INSERT INTO incoming_messages (chat_id, from_id, from_name, text, status)
		 VALUES ($1, $2, $3, $4, $5) RETURNING id`,
		m.ChatID, m.FromID, m.FromName, m.Text, m.Status,
	).Scan(&id)
	if err != nil {
...
// InsertOutbox queues a message for sending.
func (s *Store) InsertOutbox(ctx context.Context, chatID int64, text string) (int64, error) {
	var id int64
	err := s.pool.QueryRow(ctx,
		`INSERT INTO outbox (chat_id, text, status) VALUES ($1, $2, 'pending') RETURNING id`,
		chatID, text,
	).Scan(&id)
	if err != nil {
...
func (s *Store) ClaimNextOutbox(ctx context.Context, now time.Time, retryBackoff int64) (*OutboxItem, error) {
	...
	err := s.pool.QueryRow(ctx, `
		...
		RETURNING id, chat_id, text, status, attempts, error_message, created_at`,
		now, retryBackoff,
	).Scan(&item.ID, &item.ChatID, &item.Text, &item.Status, &item.Attempts, &item.ErrorMessage, &item.CreatedAt)
	if err != nil {
...
// MarkOutboxSent marks an outbox row as sent.
func (s *Store) MarkOutboxSent(ctx context.Context, id int64) error {
	_, err := s.pool.Exec(ctx,
		`UPDATE outbox SET status = 'sent', error_message = '', updated_at = now() WHERE id = $1`, id)
	if err != nil {
...
func (s *Store) ListIncoming(ctx context.Context, limit int) ([]Message, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT id, chat_id, from_id, from_name, text, status, created_at
	...
	for rows.Next() {
		var m Message
		if err := rows.Scan(&m.ID, &m.ChatID, &m.FromID, &m.FromName, &m.Text, &m.Status, &m.CreatedAt); err != nil {
...
func (s *Store) QueryIncoming(ctx context.Context, f IncomingFilter) ([]Message, error) {
	...
	query := `SELECT id, chat_id, from_id, from_name, text, status, created_at
...
func (s *Store) GetOutbox(ctx context.Context, id int64) (*OutboxItem, error) {
	var item OutboxItem
	err := s.pool.QueryRow(ctx, `
		SELECT id, chat_id, text, status, attempts, error_message, created_at, updated_at
		FROM outbox WHERE id = $1`, id,
	).Scan(&item.ID, &item.ChatID, &item.Text, &item.Status, &item.Attempts, &item.ErrorMessage, &item.CreatedAt, &item.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
